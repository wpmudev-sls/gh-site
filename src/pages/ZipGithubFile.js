...
const ZipPage = () => {
  ...
  /**
   * On component mount chcek if there is a github url set in query params
   * The query param to check is : gh-url
   */
  useEffect ( ()=>{
    // Check if github url is passed through url param
    const queryString = window.location.search,
      urlParams = new URLSearchParams(queryString);

      if ( isVarSet( urlParams.get('gh-url' ) ) ) {
        document.getElementById("inputURL").value = urlParams.get('gh-url' );
        handleClick();
      }

      //urlParams.delete( 'gh-url' );
      window.history.pushState({}, "", document.location.href.split("?")[0]);
  }, []);

  /**
   * Helper function to check if variable is set.
   * 
   * @param {*} inputVar 
   * @returns 
   */
  const isVarSet = (inputVar) => {
    return typeof inputVar !== 'undefined' && inputVar !== null;
}
  ...
  const validateInputUrl = (input) => {
    // We expect url to have a file extension.
    // We also allow only php, js and css files.
    const allowedExtensions = ['php', 'js', 'css'],
      extension = input.split('.').pop(),
      validExtension = isVarSet( extension ) && allowedExtensions.includes( extension );
